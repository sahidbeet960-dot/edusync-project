...
const SharedTimetable = () => {
  ...
  const handleSaveSlot = (e) => {
    ...
    const existingClassIndex = currentDaySchedule.findIndex(
      (c) => c.id === slotFormData.id,
    );
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dayEvents = calendarEvents.filter(
        (evt) => new Date(evt.event_date).toDateString() === dateString,
      );
      ...
      days.push(
        ...
        >
          <span
            className={`text-sm font-bold ${isToday ? "text-teal-700" : "text-slate-600"}`}
          >
            {day}
          </span>
          ...
          <div className="mt-1 w-full space-y-1 overflow-hidden">
            {dayEvents.slice(0, 2).map((evt, idx) => (
              <div
                key={idx}
                className="text-[9px] sm:text-[10px] truncate bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-bold w-full"
              >
                {evt.title}
              </div>
            ))}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 flex items-center">
            <CalendarDays className="w-6 h-6 mr-3 text-teal-600" /> Weekly
            Schedule
          </h2>
          <p className="text-slate-500 mt-1 text-sm">
            {canEdit
              ? "Modifications here will instantly update the student dashboard."
              : "Your current class timetable."}
          </p>
        </div>
            ...
            <div className="p-4 flex-1 space-y-4">
              {!(schedule[day] && schedule[day].length > 0) ? (
                ...
                schedule[day].map((cls) => (
                  <div
                    key={cls.id}
                    className="relative group border border-slate-200 rounded-xl p-4 hover:border-teal-300 hover:shadow-md transition-all bg-white"
                  >
                    <div
                      className={`absolute left-0 top-0 bottom-0 w-1.5 rounded-l-xl ${cls.type === "Lab" ? "bg-purple-400" : "bg-teal-400"}`}
                    ></div>
                    ...
                    {canEdit && (
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-1 bg-white shadow-sm rounded-md border border-slate-100 p-1">
                        <button
                          onClick={() => handleEditClass(day, cls)}
                    ...
                    <h4 className="font-bold text-slate-800 text-sm pr-12 leading-tight">
                      {cls.subject}
                    </h4>
                    <span
                      className={`inline-block mt-1.5 mb-3 px-2 py-0.5 text-[10px] font-bold uppercase rounded-md tracking-wider ${
                        cls.type === "Lab"
        ...
        <div className="fixed bottom-8 right-8 bg-slate-800 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center animate-in slide-in-from-bottom-5 fade-in duration-300 z-50">
          ...
          <div>
            ...
            <p className="text-xs text-slate-300 mt-0.5">
              Students can now see the updated timetable.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-200 bg-slate-50">
              <h3 className="text-xl font-bold text-slate-800">
                {slotFormData.subject ? "Edit Class" : "Add New Class"}{" "}
                <span className="text-teal-600">({activeDay})</span>
              </h3>
              <button
                onClick={() => setIsSlotModalOpen(false)}
                className="text-slate-400 hover:text-slate-700 bg-white p-1 rounded-full shadow-sm"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveSlot} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-teal-500 bg-slate-50"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSlotFormData({ ...slotFormData, type: e.target.value })
                    }
                    className="w-full p-3 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-teal-500 bg-slate-50"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSlotFormData({ ...slotFormData, room: e.target.value })
                    }
                    className="w-full p-3 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-teal-500 bg-slate-50"
              ...
              <div className="pt-4 flex space-x-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setIsSlotModalOpen(false)}
                  className="flex-1 py-2.5 text-slate-600 font-bold rounded-xl hover:bg-slate-100 transition-colors"
                >
                  Cancel
                </button>
                <button
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-6xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b border-slate-200 bg-slate-50 shrink-0">
              <h3 className="text-xl font-bold text-slate-800 flex items-center">
                <Calendar className="w-6 h-6 mr-3 text-indigo-600" /> Academic
                Calendar
              </h3>
              <button
                onClick={() => {
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
              ...
              <div
                className={`p-6 overflow-y-auto ${canEdit || selectedDateEvents ? "lg:w-2/3 border-b lg:border-b-0 lg:border-r border-slate-200" : "w-full"}`}
              >
                {/* Calendar Controls */}
                ...
                <div className="p-6 lg:w-1/3 bg-slate-50 overflow-y-auto flex flex-col space-y-6">
                      ...
                      <div className="space-y-4">
                        {selectedDateEvents.events.map((evt) => (
                          ...
                          >
                            ...
                            {evt.description && (
                              <p className="text-xs text-slate-600 mt-2 mb-3 bg-white p-2 rounded border border-slate-100">
                                {evt.description}
                              </p>
                            )}
                            <div className="space-y-1.5 mt-2">
                              <div className="flex items-center text-[11px] font-bold text-indigo-700">
                                ...
                                {new Date(evt.event_date).toLocaleTimeString(
                                  [],
                  ...
                  {canEdit && (
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm mt-auto">
                      <h4 className="font-bold text-slate-700 mb-4 uppercase tracking-wider text-xs border-b border-slate-100 pb-2">
                        Add New Event
                      </h4>
                      <form onSubmit={handleCreateEvent} className="space-y-4">
                        <div>
                          <input
                            ...
                            className="w-full p-2.5 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm bg-slate-50"
                        ...
                        <div>
                          <input
                            ...
                            className="w-full p-2.5 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm text-slate-600 bg-slate-50"
                          />
                        </div>
                        <div>
                          <input
                        ...
                        <div>
                          <textarea
                            ...
                            className="w-full p-2.5 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm min-h-[60px] bg-slate-50"
                        ...
                        <button
                          ...
                          className="w-full py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-70 flex items-center justify-center text-sm"
                        >
                          {isSubmittingEvent ? (
