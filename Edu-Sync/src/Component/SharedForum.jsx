...
const SharedForum = () => {
  ...
  const handleVote = async (answerId, voteType) => {
    ...
    try {
      const response = await apiClient.post(
        `/api/v1/forum/answers/${answerId}/vote`,
        {
          vote: newVote,
        },
      );
  ...
  const handleToggleVerify = async (answerId, currentStatus) => {
    ...
    try {
      ...
      alert(
        error.response?.data?.detail || "Failed to update official status.",
      );
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 flex items-center">
            <MessageCircleQuestion className="w-6 h-6 mr-3 text-indigo-600" />{" "}
            Doubt Forum
          </h2>
                    ...
                    {isExpanding ? (
                      ...
                      <div className="space-y-6">
                        {expandedData?.answers?.length > 0 ? (
                          <div className="space-y-4">
                            <h4 className="font-bold text-slate-700 text-sm uppercase tracking-wider mb-4">
                              Answers ({expandedData.answers.length})
                            </h4>

                            {expandedData.answers.map((ans) => {
                              const isVerified = ans.is_professor_verified;
                              const answerAuthor = getAuthorName(ans, "User");
                              const answerInitial = answerAuthor
                                .charAt(0)
                                .toUpperCase();
                              ...
                              return (
                                <div
                                  key={ans.id}
                                  className={`p-4 rounded-xl border transition-all ${isVerified ? "bg-green-50/50 border-green-300 shadow-md" : "bg-white border-slate-200 shadow-sm"}`}
                                >
                                  <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center">
                                      <div
                                        className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm mr-3 shadow-sm transition-colors ${isVerified ? "bg-green-600" : "bg-indigo-500"}`}
                                      >
                                        {answerInitial}
                                      </div>
                                      <div>
                                  ...
                                  <p className="text-sm text-slate-700 whitespace-pre-wrap pl-11">
                                    {ans.content}
                                  </p>
                                  ...
                                  <div className="mt-4 pl-11 flex items-center flex-wrap gap-2">
                                    ...
                                    >
                                      <ThumbsUp
                                        className={`w-4 h-4 mr-1.5 ${userVotes[ans.id] === 1 ? "fill-indigo-600 text-indigo-600" : ""}`}
                                      />
                                      {ans.upvotes || 0}
                                    </button>
                                    ...
                                    >
                                      <ThumbsDown
                                        className={`w-4 h-4 mr-1.5 ${userVotes[ans.id] === -1 ? "fill-red-600 text-red-600" : ""}`}
                                      />
                                      {ans.downvotes || 0}
                                    </button>
                                    ...
                                    {userRole === "PROFESSOR" && (
                                      <button
                                        onClick={() =>
                                          handleToggleVerify(ans.id, isVerified)
                                        }
                                      ...
                                      >
                                        <ShieldCheck
                                          className={`w-4 h-4 mr-1.5 ${isVerified ? "text-green-600" : ""}`}
                                        />
                                        {isVerified
                                          ? "Unmark Official"
                                          : "Mark Official"}
                        ...
                        <form
                          ...
                          className="mt-6 flex flex-col items-end border-t border-slate-200 pt-6"
                        >
                          <textarea
                            required
                            value={newAnswer}
            ...
            <form onSubmit={handleAskQuestion} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end space-x-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="px-5 py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-70 flex items-center"
                >
                  {isSubmitting ? (
                    <Loader2 className="w-4 h-4 animate-spin mr-2" />
                  ) : null}{" "}
