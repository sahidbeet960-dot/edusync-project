...
const SharedResources = () => {
  ...
  const fetchMaterials = async () => {
    ...
    try {
      ...
      const verifiedNotesOnly = response.data.filter(
        (file) =>
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      ...
      if (userRole === "PROFESSOR" || userRole === "CR") {
        ...
        alert(
          "Upload successful! It has been sent to the Professor for verification.",
        );
      }
      ...
      console.warn(
        "Upload error or backend serialization issue. Refreshing feed anyway...",
      );
      setIsUploadModalOpen(false);
      setUploadForm({
        title: "",
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 flex items-center">
            <Library className="w-6 h-6 mr-3 text-blue-600 fill-blue-100" />{" "}
            Academic Vault
          </h2>
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center text-slate-500 bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-sm mt-1">
            No verified materials have been uploaded yet.
          </p>
        </div>
      ) : (
        <div className="animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {Object.keys(structuredData)
                ...
                .map((sem) => {
                  ...
                  const fileCount = Object.values(structuredData[sem]).flat()
                    .length;
                  return (
                    <div
                      key={sem}
          ...
          {activeSemester && !activeFolder && (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {Object.keys(structuredData[activeSemester])
                .sort()
                .map((subject) => (
                  <div
                    key={subject}
          ...
          {activeSemester && activeFolder && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {structuredData[activeSemester][activeFolder]?.map((file) => (
                <div
                  key={file.id}
                  className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col relative group"
                >
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {(userRole === "PROFESSOR" || userRole === "CR") && (
                        <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-50 p-1 rounded-lg border border-slate-200 shadow-sm">
                          <button
                            onClick={() => handleUnverify(file.id)}
                            className="p-1.5 text-orange-500 hover:bg-orange-100 rounded-md"
                            title="Unverify"
                          >
                      ...
                      <a
                        ...
                        className="p-2 text-slate-400 hover:text-blue-600 bg-slate-50 hover:bg-blue-50 rounded-lg transition-colors border border-transparent group-hover:border-slate-200"
                        title="Download"
                      >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-slate-200 bg-slate-50">
              <h3 className="text-xl font-bold text-slate-800 flex items-center">
                <FilePlus className="w-5 h-5 mr-2 text-blue-600" /> Upload
                Material
              </h3>
              <button
            ...
            <form onSubmit={handleUploadSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 min-h-[80px]"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200 border border-slate-300 rounded-xl p-2 bg-slate-50 cursor-pointer"
                />
              </div>
              ...
              <div className="pt-4 flex justify-end space-x-3 border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors flex items-center shadow-md disabled:opacity-70"
                >
                  {isUploading ? (
