...
const StudentHome = () => {
  ...
  const fetchAllDashboardData = async () => {
    ...
    try {
      ...
      const verifiedStudyMaterials = materialsRes.data.filter(
        (file) =>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 ease-in-out relative max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-blue-700 p-6 rounded-2xl shadow-md flex items-center text-white relative overflow-hidden">
          ...
          <div className="p-4 bg-white/20 backdrop-blur-sm text-white rounded-xl mr-4">
        ...
        <div
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center cursor-pointer hover:border-indigo-300 transition-all hover:shadow-md"
        ...
        <div
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center cursor-pointer hover:border-teal-300 transition-all hover:shadow-md"
          onClick={() => navigate("/dashboard/student/discussions")}
        >
        ...
        <div
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center cursor-pointer hover:border-amber-300 transition-all hover:shadow-md"
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col sm:flex-row items-center justify-between gap-6 overflow-hidden relative">
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  Collaborate in real-time and track your hours.
                </p>
              </div>
            </div>
            ...
            <form
              ...
              className="flex w-full sm:w-auto relative z-10"
            >
              <input
                type="text"
            ...
            <div className="space-y-4">
              {todaysClasses.length === 0 ? (
                <div className="text-center py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                  <div className="inline-block p-4 bg-green-100 text-green-600 rounded-full mb-3">
                  ...
                  <p className="text-slate-400 text-sm">
                    Perfect time to hit a study room!
                  </p>
                </div>
              ) : (
                todaysClasses.map((cls, index) => (
                  <div key={cls.id || index} className="flex relative group">
                    {index !== todaysClasses.length - 1 && (
                      <div className="absolute left-6 top-10 bottom-[-16px] w-0.5 bg-slate-200 group-hover:bg-indigo-300 transition-colors"></div>
                    )}
                    ...
                    <div className="relative flex-1 bg-slate-50 hover:bg-indigo-50 transition-all border border-slate-100 hover:border-indigo-200 p-5 rounded-xl ml-2 shadow-sm">
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1.5 rounded-l-xl ${cls.type === "Lab" ? "bg-purple-500" : "bg-indigo-500"}`}
                      ></div>
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex flex-col items-end space-y-2">
                          <span
                            className={`px-3 py-1 text-[10px] font-bold uppercase rounded-full tracking-wider shadow-sm ${
                              cls.type === "Lab"
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-3">
              ...
              <button
                onClick={() => navigate("/dashboard/student/schedule")}
                className="text-xs font-bold text-indigo-600 hover:underline bg-indigo-50 px-3 py-1.5 rounded-full"
              >
                View All
              </button>
            </div>
            <div className="space-y-3">
              {upcomingEvents.length === 0 ? (
                <p className="text-sm text-slate-500 italic text-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                upcomingEvents.map((ev) => {
                  ...
                  return (
                    <div
                      key={ev.id}
                      className="flex items-center p-3.5 bg-slate-50 rounded-xl border border-slate-100 hover:border-indigo-200 transition-all hover:shadow-sm"
                    >
                      <div
                        className={`p-2.5 rounded-xl mr-4 shrink-0 shadow-sm ${style.bg} ${style.text}`}
                      >
                        {style.icon}
                      </div>
                      <div>
